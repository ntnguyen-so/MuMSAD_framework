...
class HBOS(BaseDetector):
    ...
    def __init__(self, n_bins=10, alpha=0.1, tol=0.5, contamination=0.1):
        super(HBOS, self).__init__(contamination=max(0.01,min(contamination, 0.499)))
        self.n_bins = n_bins
        self.alpha = alpha
        self.tol = tol
    ...
    def fit(self, X, y=None):
        ...
        self.decision_scores_per_var = outlier_scores.copy()
        for i in range(self.decision_scores_per_var.shape[1]):
            self.decision_scores_per_var[:, i] = invert_order(self.decision_scores_per_var[:, i])
        max_score, min_score = np.max(self.decision_scores_per_var), np.min(self.decision_scores_per_var)
        #self.decision_scores_per_var = MinMaxScaler().fit_transform(self.decision_scores_per_var)
        for i in range(self.decision_scores_per_var.shape[1]):
            self.decision_scores_per_var[:, i] = (self.decision_scores_per_var[:, i] - min_score) / (max_score - min_score) # invert_order(self.decision_scores_per_var[:, i])
        self.decision_scores_ = np.sum(self.decision_scores_per_var, axis=1)        
        return self
