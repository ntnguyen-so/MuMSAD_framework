...
from lof_core import LocalOutlierFactor
...
class LOF(BaseDetector):
    ...
    def __init__(self, n_neighbors=20, algorithm='auto', leaf_size=30,
        ...
        super(LOF, self).__init__(contamination=min(.49,contamination))
    ...
    def fit(self, X, y=None):
        ...
        self.decision_scores_per_var_ = np.zeros_like(self.detector_.negative_outlier_factor_per_var_)
        for col in range(self.detector_.negative_outlier_factor_per_var_.shape[1]):
            self.decision_scores_per_var_[col] = invert_order(
            self.detector_.negative_outlier_factor_per_var_[col])
        self.anomaly_ranking_ = np.argsort(-self.decision_scores_per_var_, axis=1)
