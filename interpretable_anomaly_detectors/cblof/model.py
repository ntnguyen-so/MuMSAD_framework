...
class CBLOF(BaseDetector):
    ...
    def __init__(self, n_clusters=8, contamination=0.1,
                 ...
                 n_jobs=1):
        super(CBLOF, self).__init__(contamination=min(.49,contamination))
    ...
    def fit(self, X, y=None):
        ...
        self.decision_scores_, self.decision_scores_per_var_ = self._decision_function(X,
    ...
    def _decision_function(self, X, labels):
        ...
        scores_per_var = np.zeros_like(X)
        ...
        if small_indices.shape[0] != 0:
            ...
            dist_to_large_center_min_indices = np.argmin(dist_to_large_center, axis=1)
            i = 0
            for small_index in small_indices:
                scores_per_var[small_index] = np.square(X[small_index, :] - 
                                                        self._large_cluster_centers[dist_to_large_center_min_indices[i]])
                i += 1            
        ...
        if large_indices.shape[0] != 0:
            ...
            scores_per_var[large_indices] = np.square(X[large_indices, :], large_centers)

        if self.use_weights:
            # Weights are calculated as the number of elements in the cluster
            scores = scores * self.cluster_sizes_[labels]
            scores_per_var *= self.cluster_sizes_[labels]

        scores_per_var = MinMaxScaler().fit_transform(scores_per_var)
        scores = np.sum(scores_per_var, axis=1)
        return scores.ravel(), scores_per_var