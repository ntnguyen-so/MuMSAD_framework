...
def ravel_without_specified_axis(arr, axis):
    # Get the shape of the array
    shape = arr.shape
    # Calculate the shape of the array after flattening along the specified axis
    new_shape = (shape[axis], -1)
    # Reshape the array to move the specified axis to the first position
    reshaped_arr = np.moveaxis(arr, axis, 0)
    # Reshape the array to flatten along the first axis
    raveled_arr = reshaped_arr.reshape(new_shape)
    raveled_arr = np.swapaxes(raveled_arr, 0, 1)
    return raveled_arr
...
def main(config: AlgorithmArgs):
    ...
    if config.runtimeOutput:
        ...
        with open(config.runtimeOutput, 'w') as f:
            f.write('Total:{0}\n'.format(total_time))
    ...
    errors = []
    for preds, labels in zip(predictions, targets):
        error = np.abs(labels - preds).mean(axis=0)
        errors.append(error)
    print(f"{len(predictions)}x error shape: {error.shape}")
    scores, _, _, _ = sliding_score(np.array(errors),
                                    small_window=config.customParameters.scoring_small_window_size,
                                    large_window=config.customParameters.scoring_large_window_size)
    scores = np.concatenate([
        # the first batch of training samples has no predictions --> no scores
        np.full(shape=(params.train_length, config.customParameters.context_window_size, series.shape[1]), fill_value=np.nan),
        scores
    ], axis=0)
    print(scores.shape)
    scores = 1 - ravel_without_specified_axis(scores, axis=2)# scores.ravel()
    if padding_needed:
        # remove padding points
    ...
    pd.DataFrame(scores).to_csv(config.anomalyScorePerVarOutput, index=False, header=None)
