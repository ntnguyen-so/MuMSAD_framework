...
class LocalOutlierFactor(KNeighborsMixin, OutlierMixin, NeighborsBase):
    ...
    def kneighbors_per_var(self, X, neighbors_indices):
        distances_per_var = []
        for i in range(len(X)):
            X_i = X[i]
            neighbors_X_i = neighbors_indices[i]
            distances_per_var_from_X_i = []
            for neighbor_X_i in neighbors_X_i:
                distances_per_var_from_X_i.append(np.square(X_i - X[neighbor_X_i]))
            distances_per_var.append(distances_per_var_from_X_i)
        return np.array(distances_per_var)
    ...
    def fit(self, X, y=None):
        ...
        self._distances_fit_X_per_var = self.kneighbors_per_var(X, _neighbors_indices_fit_X_)
        ...
        self._lrd_per_var = self._local_reachability_density_per_var(
            self._distances_fit_X_per_var, _neighbors_indices_fit_X_)

        # Compute lof score over training samples to define offset_:
        lrd_ratios_array = (
            self._lrd[_neighbors_indices_fit_X_] / self._lrd[:, np.newaxis]
        )
        lrd_ratios_array_per_var = (
            self._lrd_per_var[_neighbors_indices_fit_X_] / self._lrd_per_var[:, np.newaxis]
        )

        self.negative_outlier_factor_ = -np.mean(lrd_ratios_array, axis=1)
        self.negative_outlier_factor_per_var_ = -np.mean(lrd_ratios_array_per_var, axis=1)
    ...
    def _local_reachability_density_per_var(self, distances_X_per_var, neighbors_indices):
        """The local reachability density (LRD) per var
        ...
        local_reachability_density : ndarray of shape (n_queries,)
            The local reachability density of each sample.
        """
        dist_k_per_var = self._distances_fit_X_per_var[neighbors_indices, self.n_neighbors_ - 1]
        reach_dist_array = np.maximum(dist_k_per_var, distances_X_per_var)
